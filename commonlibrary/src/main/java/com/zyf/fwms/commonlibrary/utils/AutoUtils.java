...
public class AutoUtils {
	...
	public static void setSize(Activity act, boolean hasStatusBar, int designWidth, int designHeight){
		...
		//缩放比例
		float w = (float)displayWidth / (float) designWidth;
		float h = (float)displayHeight / (float) designHeight;
		scaling=w>=h?w:h;
	}
    ...
    public static void auto(View view){
		...
		if(view instanceof TextView) {
			...
			if(typeface!=null&&typeface.getStyle()==Typeface.BOLD)//如果是加粗 就采用正常字体
				((TextView) view).setTypeface(Typeface.DEFAULT);
			else
				CommonUtils.setTextViewFont((TextView) view, R.font.sourcehansanscn_light);
...
//        return designWidthValue * displayWidth / designWidth;
		return (int)(designWidthValue * scaling);
...
//		return designHeightValue * displayHeight / designHeight;
		return (int)(designHeightValue * scaling);
    }
}
