            ...
            case BaseResp.ErrCode.ERR_OK:
                if (1 == TYPE_LOGIN) {
                    if (resp instanceof SendAuth.Resp) {
                        ...
                        // if (null != sILoginCallback) sILoginCallback.tokeCallBack(code);
                        ThreadManager.getShortPool().execute(new Runnable() {
                            @Override
                            public void run() {//获取token 和用户信息
                                ...
                                sILoginCallback = null;
                                WeiXinLogin.setLoginCallback(null);
                            }
                        });
                    }
                } else {
